...
class Monitor():
    def __init__(self):
        ...
        """
        Los turnos 0, 1 y 2 corresponden respectivamente a los peatones, coches hacia el norte y coches hacia el sur.
        """
        self.turn = Value('i', 0)
    ...
    def okpedestrians(self):
        return (self.ncarNorth.value == 0 and self.ncars_south.value == 0) and \
            (self.turn.value == 0 or (self.ncars_south_waiting.value == 0 and self.ncars_north_waiting.value == 0))
    
    def okcarsnorth(self):
        return self.npedestrians.value == 0 and self.ncars_south.value == 0 and \
            ((self.turn.value == 1) or (self.ncars_south_waiting.value == 0 and self.npedestrians_waiting.value == 0))      
    
    def okcarssouth(self):
        return self.npedestrians.value == 0 and self.ncarNorth.value == 0 and \
             (self.turn.value == 2 or (self.ncars_north_waiting.value == 0 and self.npedestrians_waiting.value == 0))
    ...
    def wants_enter_car(self, direction: int) -> None:
        ...
        if direction == NORTH:
            self.ncars_north_waiting.value +=1
            self.oktoN.wait_for(self.okcarsnorth)
            self.ncars_north_waiting.value -=1
            self.ncarNorth.value += 1
        else:
            self.ncars_south_waiting.value +=1
            self.oktoS.wait_for(self.okcarssouth)
            self.ncars_south_waiting.value -=1
